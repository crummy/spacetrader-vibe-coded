// Ship Purchase Screen - Ship purchasing functionality (accessed from Shipyard)
...
export function ShipPurchaseScreen({ onNavigate, onBack, state, onAction, availableActions }: ScreenProps) {
  ...
  const handlePurchase = async () => {
    console.log('🔄 HandlePurchase called, selectedShip:', selectedShip);
    ...
    if (!ship) {
      console.log('❌ Ship not found in availableShips:', selectedShip);
      return;
    }
    console.log('🚢 Ship to purchase:', ship.name, 'netPrice:', ship.netPrice);
    ...
    if (!ship.canAfford) {
      if (ship.netPrice < 0) {
        // This shouldn't happen with negative prices, but just in case
        setMessage('Unable to complete trade - please contact support');
      } else {
    ...
    try {
      console.log('🎬 Executing buy_ship action with shipType:', selectedShip);
      const result = await actualExecuteAction({
        type: 'buy_ship',
        parameters: {
      ...
      if (result.success) {
        if (ship.netPrice < 0) {
          const cashBack = Math.abs(ship.netPrice);
          setMessage(`Successfully traded for ${ship.name} and received ${cashBack.toLocaleString()} cr cash back!`);
        } else {
          ...
          <div className="bg-space-dark border border-neon-cyan rounded p-4 max-w-sm w-full mx-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="border-t border-palm-gray border-opacity-30 pt-3 mb-4">
              <div className="text-xs text-palm-gray space-y-1">
                ...
                <span>{selectedShipData.netPrice < 0 ? 'Cash Back:' : 'Net Cost:'}</span>
                <span className="text-neon-green">
                    {selectedShipData.netPrice < 0 
                       ? `+${Math.abs(selectedShipData.netPrice).toLocaleString()}` 
                       : selectedShipData.netPrice.toLocaleString()} cr
                   </span>
                 </div>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => {
                  console.log('🖱️ Buy Ship button clicked!');
                  handlePurchase();
                }}
